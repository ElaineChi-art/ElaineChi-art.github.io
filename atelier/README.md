...
| `course/` | 針織跑鞋 | **放樣**（loft）—— 鞋沒有旋轉軸，車削做不出來 |

前三個都能用 `LatheGeometry`（剖面繞軸旋轉）解決。**鞋不行** —— 它沒有旋轉對稱軸。所以 `course/` 用的是斷面放樣：沿鞋楦從腳跟走到腳尖，每一站定義半寬 `WIDTH` 與拱高 `HEIGHT`，斷面用超橢圓（側面近垂直、頂面平緩，才像鞋不像香腸），再把所有站點縫成一張曲面。中底／大底／鞋墊則是同一條鞋型輪廓做不同 inset 的擠出。

布料的關鍵是 `sheen` —— 沒有它，針織會看起來像塑膠。

⚠️ `TorusGeometry(r, tube)` 的 `tube` 若接近或大於 `r`，環會塌成一團；而且**不能用非等比縮放把圓環壓成橢圓**（管徑會跟著變形成刀片狀）。要橢圓環就用 `TubeGeometry` 沿橢圓曲線掃。
